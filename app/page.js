'use client';
    ...
    for (let iter = 0; iter < 120; iter++) {
      ...
      for (let i = 0; i < photos.length; i++) {
        for (let j = i + 1; j < photos.length; j++) {
          ...
          // Overlap depths on each axis (positive = overlapping by that many 
  px)
  ...
  export default function Home() {
    ...
    // Mouse wheel / trackpad scroll support (drag-to-pan alone misses this 
  input)
    useEffect(() => {
      ...
      function handleWheel(e) {
        ...
        const nx = Math.max(-(cw - vw), Math.min(0, offRef.current.x -
  e.deltaX));
        const ny = Math.max(-(ch - vh), Math.min(0, offRef.current.y -
  e.deltaY));
    ...
    function stopMomentum() {
      if (rafRef.current) { cancelAnimationFrame(rafRef.current); rafRef.current
  = null; }
    }
    ...
    function startMomentum(vx, vy) {
      ...
      function tick() {
        velX *= 0.93;
        velY *= 0.93;
        if (Math.abs(velX) < 0.08 && Math.abs(velY) < 0.08) { rafRef.current =
  null; return; }
        const { w: cw, h: ch } = cRef.current;
        const vw = window.innerWidth, vh = window.innerHeight;
    ...
    function onPointerDown(e) {
      ...
      startP.current  = { x: e.clientX - offRef.current.x, y: e.clientY -
  offRef.current.y };
    ...
    function onPointerMove(e) {
      ...
      const { w: cw, h: ch } = cRef.current;
      const vw = window.innerWidth, vh = window.innerHeight;
      const nx = Math.max(-(cw - vw), Math.min(0, e.clientX -
  startP.current.x));
      const ny = Math.max(-(ch - vh), Math.min(0, e.clientY -
  startP.current.y));
      const dx = Math.abs(nx - offRef.current.x) + Math.abs(ny -
  offRef.current.y);
